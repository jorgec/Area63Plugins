...
class Media_Uploader extends \WP_Shopify\Processing\Vendor_Background_Process
{
   protected $action = 'wps_background_processing_media_uploader';
...
/*
   ...
   public function get_existing_attachments() {
      ...
      $existing_attachments = \wp_cache_get('wpshopify_existing_attachments');
      ...
      if  (!empty($all_images_meta)) {
         \wp_cache_add('wpshopify_existing_attachments', $all_images_meta);
      }
	...
	*/
	public function task($image) {
      ...
      $process_media_results = $this->process_media($image, $existing_attachments);
   ...
   protected function complete() {
      ...
      \wp_cache_delete('wpshopify_existing_attachments');
   ...
   */
   public function process_media($media_item, $existing_attachments) {
      ...
      $file_info = $this->download_media_file($media_item, $existing_attachments);
   ...
   */
   ...
   public function attachment_exists($existing_attachments, $image) {
      ...
      return array_filter($existing_attachments, function($existing_attachment) use($image) {
         return Utils::str_contains($existing_attachment->meta_value, $image->src);
      });
   ...
   public function get_attachment_id_from_existing($existing_attachment) {
      ...
      $existing_attachments = maybe_unserialize($reset_indexes[0]->meta_value);
   ...
   public function download_multiple_media_files($images, $existing_attachments) {
      
      $results = [];

      foreach ($images as $image) {
         $results[] = $this->download_media_file($image, $existing_attachments);
      }
   ...
   public function build_download_results($temp_file_path, $temp_file_path_mod, $mime_and_extension, $image) {
   ...
   */
	public function download_media_file($image, $existing_attachments) {
      ...
      $existing_attachment = $this->attachment_exists($existing_attachments, $image);

      if (!empty($existing_attachment)) {

         $existing_attachment_id = $this->get_attachment_id_from_existing($existing_attachment);
         ...
         if ($existing_attachment_id) {
            ...
            $this->DB_Settings_Syncing->update_recently_syncd_media_ref($image->src);
      ...
      */
      ...
      $temp_file_path_mod = $this->rename_temp_file($temp_file_path, $mime_and_extension);


      $result = $this->build_download_results($temp_file_path, $temp_file_path_mod, $mime_and_extension, $image);
   ...
   */
   public function upload_media_file($file) {
      ...
      if ($file['existing']) {
         ...
         $set_feat_result = \set_post_thumbnail( $file['post_id'], $file['attachment_id'] );
         $this->DB_Settings_Syncing->increment_current_amount('media');
      ...
      $file_attributes = \wp_handle_sideload( $file_array, $this->sideload_overrides() );
      ...
      $attachment_id = \wp_insert_attachment($attachment, $file_attributes['file'], $file['post_id'], true);
      ...
      $attach_data = \wp_generate_attachment_metadata( $attachment_id, $file_attributes['file'] );

      // Important
      $attach_data['wpshopify'] = true;
      $attach_data['orig_src'] = $file['orig_src'];
      $attach_data['attachment_id'] = $attachment_id;

      $update_meta_result = \wp_update_attachment_metadata( $attachment_id, $attach_data );

      $update_alt_result = \update_post_meta($attachment_id, '_wp_attachment_image_alt', $file['alt'] );

      $set_feat_result = \set_post_thumbnail( $file['post_id'], $attachment_id );
