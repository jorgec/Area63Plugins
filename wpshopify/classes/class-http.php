...
class HTTP {
	...
	*/
	public function get_shopify_api_call_amount($response) {
		...
		$call_limit = wp_remote_retrieve_header($response, WP_SHOPIFY_SHOPIFY_HEADER_API_CALL_LIMIT);
	...
	*/
	public function check_rate_limit($response) {
		...
		if ($call_total === WP_SHOPIFY_SHOPIFY_RATE_LIMIT || $call_total === false) {
	...
	*/
	public function request($method, $url, $body = false, $blocking = false) {
      ...
      // 
      $api_ver = wp_remote_retrieve_header($response, 'x-shopify-api-version');
      $dep_reason = wp_remote_retrieve_header($response, 'x-shopify-api-deprecated-reason');

      if (!empty($dep_reason)) {
         error_log('WP Shopify Warning: A response includes a resource or property that is deprecated and removed in a later API version: ' . $dep_reason);
      }

      if ($api_ver !== WP_SHOPIFY_ADMIN_REST_API_VERSION) {
         error_log('WP Shopify Warning: Admin REST API version does not match! Request returned "' . $api_ver . '" but app uses "' . WP_SHOPIFY_ADMIN_REST_API_VERSION . '"');
      }
      ...
      */
      ...
      return [
         ...
         'body' => json_decode( $json_from_response, false, 512, JSON_BIGINT_AS_STRING )
      ];
